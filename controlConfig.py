...
parameterSchema = {
    ...
    "additive": {
        ...
        "bedTemperatureLayer0":         {"type": int,   "min": 0,    "max": 150,    "default": 65,    "unit": "°C",     "required": False, "description": "首层热床温度"},
        ...
        "retractionReloadSpeed":        {"type": float, "min": 10.0, "max": 2000.0, "default": 300.0, "unit": "mm/min", "required": False, "description": "重载速度（建议300mm/min，给螺杆充足压力建立时间）"},
        "retractionReloadExtraRatio":   {"type": float, "min": 0.0,  "max": 0.5,    "default": 0.1,                     "required": False, "description": "重载超量补偿比例（补偿螺杆滞后，建议0.05~0.15）"},
        "retractionMinTravelDistance":  {"type": float, "min": 0.0,  "max": 50.0,   "default": 5.0,   "unit": "mm",     "required": False, "description": "触发回抽的最小空行程距离"},
        "firstLayerLineWidthFactor":    {"type": int,   "min": 100,  "max": 200,    "default": 120,   "unit": "%",      "required": False, "description": "首层线宽倍率（增大接触面积改善粘接）"},
        "firstLayerZOverlap":           {"type": float, "min": 0.0,  "max": 0.3,    "default": 0.1,   "unit": "mm",     "required": False, "description": "首层Z轴压入量（改善热床粘接）"},
...
class ConfigManager:
    ...
    def generateCuraConfig(self, additiveConfig: Dict[str, Any]) -> Dict[str, str]:
        ...
        return {
            ...
            "bed_temperature_layer_0":         str(additiveConfig.get("bedTemperatureLayer0", 65)),
            "print_speed":                     str(additiveConfig.get("printSpeed", 40)),
            "speed_layer_0":                   str(additiveConfig.get("printSpeedLayer0", 3)),
