...
parameterSchema = {
    "mold": {
        ...
        "optimizationCriteria": {
            "type": str,
            "default": "printability",
            "options": ["printability", "strength", "material"],
            "required": False,
        },
...
class ConfigManager:
    def __init__(self, configDir: str = "./configs"):
        ...
        logger.info(f"Configuration manager initialized. Config directory: {self.configDir}")
        ...
        if configFile.exists():
            try:
                with open(configFile, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                logger.info(f"Configuration loaded: {projectId}")
                return config
            except Exception as e:
                logger.error(f"Failed to load configuration: {e}")
                return self.getDefaultConfig()
        else:
            logger.info(f"Configuration file does not exist. Using default configuration: {projectId}")
            return self.getDefaultConfig()
    ...
    def saveConfig(self, projectId: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Configuration saved: {projectId}")
            return True
        except Exception as e:
    ...
    def validate(self, config: Dict[str, Any]) -> List[str]:
        ...
        for section in ["additive", "casting", "subtractive", "mold"]:
            if section not in config:
                errors.append(f"Missing process configuration: {section}")
                continue
            if not isinstance(config[section], dict):
                errors.append(f"Invalid process configuration format: {section}")
                continue
        ...
        for paramName, paramSchema in schema.items():
            ...
            if value is None:
                if paramSchema.get('default') is not None:
                    ...
                    errors.append(f"{section}.{paramName}: Required parameter missing")
                continue
            ...
            if isinstance(value, (int, float)):
                ...
                if minVal is not None and value < minVal:
                    errors.append(f"{section}.{paramName}: Value too small")
                if maxVal is not None and value > maxVal:
                    errors.append(f"{section}.{paramName}: Value too large")
            if 'options' in paramSchema:
                if value not in paramSchema['options']:
                    errors.append(f"{section}.{paramName}: Invalid option value")
        return errors
    ...
    def generateCuraConfig(self, additiveConfig: Dict[str, Any]) -> Dict[str, str]:
        ...
        logger.info("Generated Cura slicer configuration")
        return curaConfig
    ...
    def validateSlicingReadiness(self, config: Dict[str, Any]) -> tuple[bool, list]:
        ...
        if missingParams:
            logger.error(f"Missing required slicing parameters: {missingParams}")
