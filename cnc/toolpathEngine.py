...
import trimesh
from cnc.geometryUtils import applyRotation
...
class PointCloudIPW:
    ...
    def filterPathsLocal(self, pathsLocal: List[np.ndarray], rotToToolFrame: np.ndarray,
        ...
        for pathLocal in pathsLocal:
            dists, _ = tree.query(np.asarray(pathLocal, dtype=float), k=1,
                                  distance_upper_bound=searchRadius)
            if np.any(dists != np.inf):
                validPaths.append(np.asarray(pathLocal, dtype=float))
        return validPaths

    def updateIpwLocal(self, pathsLocal: List[np.ndarray], rotToToolFrame: np.ndarray,
                       toolRadius: float) -> None:
        if not pathsLocal or not np.any(self.activeMask):
            return
        activeWcs = self.getActivePointsWcs()
        ...
        removeSet = set()
        for pathLocal in pathsLocal:
            ballIdxList = tree.query_ball_point(
                np.asarray(pathLocal, dtype=float)[:, :2], r=toolRadius)
...
class TrimeshToolpathEngine:
    def buildFlatEndMillTool(self, toolParams: Dict[str, Any]) -> FlatEndMillTool:
        ...
        shankRadius = float(toolParams.get("shankDiameter",
                                           toolParams.get("diameter", 6.0))) * 0.5
        return FlatEndMillTool(radius, length, shankRadius)

    def buildSdfForMesh(self, mesh: trimesh.Trimesh, voxelSize: float,
                        backendName: str = "auto") -> SdfVolume:
        return buildSdfVolume(mesh, voxelSize, backendName)

    def buildSweptCollisionEngine(self, tool: FlatEndMillTool,
                                  protectMeshes: List[trimesh.Trimesh],
                                  clearances: List[float], voxelSize: float,
                                  backendName: str = "auto",
                                  diskCount: int = 16, ringCount: int = 6,
                                  safeBuffer: float = 2.0) -> SweptVolumeCollisionEngine:
        ...
        return SweptVolumeCollisionEngine(tool, sdfList, clearances,
                                         diskCount=diskCount, ringCount=ringCount,
                                         safeBuffer=safeBuffer)

    def buildSweptCollisionEngineWithOffset(self, tool: FlatEndMillTool,
                                            protectMeshes: List[trimesh.Trimesh],
                                            clearances: List[float],
                                            offsets: List[float], voxelSize: float,
                                            backendName: str = "auto",
                                            diskCount: int = 16, ringCount: int = 6,
                                            safeBuffer: float = 2.0) -> SweptVolumeCollisionEngine:
        sdfList = []
        for mesh, offset in zip(protectMeshes, offsets):
            if offset != 0.0:
                sdfList.append(buildOffsetSdf(mesh, offset, voxelSize, backendName))
            else:
                sdfList.append(buildSdfVolume(mesh, voxelSize, backendName))
        return SweptVolumeCollisionEngine(tool, sdfList, clearances,
                                         diskCount=diskCount, ringCount=ringCount,
                                         safeBuffer=safeBuffer)

    def sampleSegmentLocal(self, startPoint: np.ndarray, endPoint: np.ndarray,
                           sampleStep: float) -> np.ndarray:
    ...
    def slicePathByPlatformZ(self, pathLocal: np.ndarray, rotBack: np.ndarray,
                             platformSafeZ: float) -> List[np.ndarray]:
        if len(pathLocal) == 0:
            return []
    ...
    def clipWcsPathsByZ(self, pathsWcs: List[np.ndarray],
                        safeZ: float) -> List[np.ndarray]:
        result = []
        for pathWcs in pathsWcs:
            arr = np.asarray(pathWcs, dtype=float)
            if len(arr) < 2:
                continue
            validMask = arr[:, 2] >= safeZ
            current = []
            for i, isValid in enumerate(validMask):
                if isValid:
                    ...
                    if len(current) >= 2:
                        result.append(np.asarray(current, dtype=float))
                    current = []
            if len(current) >= 2:
                result.append(np.asarray(current, dtype=float))
        return result